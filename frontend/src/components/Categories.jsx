    ...
    <div className="flex items-center gap-2 flex-wrap">
      {PRESET_COLORS.map(c => (
        <button key={c} type="button" onClick={() => onChange(c)}
          className={`w-7 h-7 rounded-full transition-transform ${value === c ? 'scale-125 ring-2 ring-offset-2 ring-zinc-400 dark:ring-offset-zinc-900' : 'hover:scale-110'}`}
          style={{ backgroundColor: c }}
        />
      ))}
      <label
        className={`w-7 h-7 rounded-full flex items-center justify-center cursor-pointer transition-transform ${isCustom ? 'scale-125 ring-2 ring-offset-2 ring-zinc-400 dark:ring-offset-zinc-900' : 'border-2 border-dashed border-slate-300 dark:border-zinc-600 hover:scale-110'}`}
...
const hdr = 'w-8 h-8 flex items-center justify-center rounded-lg text-slate-500 dark:text-zinc-400 hover:bg-slate-100 dark:hover:bg-zinc-800 transition-colors text-lg leading-none'

export default function Categories({ categories, onDone }) {
  ...
  return (
    <div className="flex flex-col h-screen bg-slate-50 dark:bg-zinc-950">
      <div className="flex items-center gap-3 px-4 py-3 bg-white dark:bg-zinc-900 border-b border-slate-200 dark:border-zinc-700 shrink-0">
        <button onClick={back} className={hdr}>‹</button>
            ...
            {editingId === cat.id ? (
              <div className="p-3 space-y-3">
                ...
                <div className="flex gap-2">
                  <button onClick={() => handleSave(cat.id)} disabled={!editName.trim()} className="flex-1 py-2 text-xs font-medium bg-indigo-600 text-white rounded-lg hover:bg-indigo-700 disabled:opacity-40 transition-colors">Save</button>
                  <button onClick={cancelEdit} className="flex-1 py-2 text-xs font-medium bg-slate-100 dark:bg-zinc-800 text-slate-600 dark:text-zinc-300 rounded-lg hover:bg-slate-200 dark:hover:bg-zinc-700 transition-colors">Cancel</button>
                  <button onClick={() => handleDelete(cat.id)} className="px-3 py-2 text-xs text-red-500 dark:text-red-400 hover:bg-red-50 dark:hover:bg-red-950/30 rounded-lg transition-colors">Delete</button>
                </div>
              </div>
            ) : (
              <button onClick={() => startEdit(cat)} className="w-full flex items-center gap-3 px-4 py-3 text-left hover:bg-slate-50 dark:hover:bg-zinc-800/50 transition-colors">
